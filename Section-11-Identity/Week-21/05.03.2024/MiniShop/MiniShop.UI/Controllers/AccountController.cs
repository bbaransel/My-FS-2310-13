    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel loginViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    await _signInManager.SignOutAsync();// Olası daha önceden kalmış olan cookieyi temizliyoruz.
                    var isComfirmed = await _userManager.IsEmailConfirmedAsync(user);
                    if (!isComfirmed)//Mail onayını kontrol ediyoruz.
                    {
                    ...
                    else if (result.IsLockedOut)
                    {
                        ...
                        _notyfService.Information($"Hesabınız şuanda kilitli. {timeLeft} saniye sonra tekrar deneyiniz.");
                        return View(loginViewModel);
                    }
                    else
                    {
