...
namespace MiniShop.UI.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task<IActionResult> Index(string isdeleted)
        {
            var x = isdeleted == "silinenurunler" ? true : false;
            Response<List<ProductViewModel>> result = await _productManager.GetAllNonDeletedAsync(x);
            ViewBag.ShowDeleted = x;
        ...
        public async Task<IActionResult> Create(AddProductViewModel model, IFormFile image)
        {
            if (ModelState.IsValid && model.CategoryIds.Count > 0 && image != null)
            {
                ...
                if (result.IsSucceeded) _notyfService.Success("Ürün başarı ile oluşturuldu.");
            ...
            if (model.CategoryIds.Count == 0) _notyfService.Error("En az bir kategori seçmilmedilir.");
        ...
        public async Task<IActionResult> Edit(EditProductViewModel model, IFormFile image)
        {
            ...
            if (ModelState.IsValid && model.CategoryIds.Count > 0)
            {
                ...
                if (result.IsSucceeded) _notyfService.Success("Ürün başarı ile güncellendi");
                else _notyfService.Error(result.Error);
                return RedirectToAction("Index");
            }
            ViewBag.CategoryErrorMessage = model.CategoryIds.Count == 0 ? "Herhangi bir kategori seçmeden, ürün kaydı yapılamaz" : null;
        ...
        public async Task<IActionResult> SoftDelete(int id)
        {
            ...
            if (productViewModel.Data.IsDeleted) _notyfService.Success("Ürün çöp kutusuna gönderilmiştir.");
            else _notyfService.Success("Ürün çöp kutusundan başarıyla çıkartılmıştır.");
